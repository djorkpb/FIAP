...
def calculate_accuracy(_model, df):
    try:
        ...
        if 'TUE' in X.columns:
            X.rename(columns={'TUE': 'TER'}, inplace=True)
        
        y_true_english = df['Obesity']
        y_pred_model = _model.predict(X)

        portuguese_to_english_map = {
            'Peso Normal': 'Normal_Weight', 'Sobrepeso Nível I': 'Overweight_Level_I',
            'Sobrepeso Nível II': 'Overweight_Level_II', 'Obesidade Tipo I': 'Obesity_Type_I',
            'Obesidade Tipo II': 'Obesity_Type_II', 'Obesidade Tipo III': 'Obesity_Type_III',
            'Peso Insuficiente': 'Insufficient_Weight'
        }
        
        y_pred_english = []
        for label in y_pred_model:
            if label in portuguese_to_english_map:
                 y_pred_english.append(portuguese_to_english_map[label])
            else:
                 y_pred_english.append(label)

        accuracy = accuracy_score(y_true_english, y_pred_english)
        return accuracy
    except Exception as e:
...
# Se o modo for "Sistema Preditivo", exibe todos os seus componentes.
if app_mode == "Sistema Preditivo":
    ...
    with metric_col:
        if accuracy is not None:
            st.metric(label="Acurácia do Modelo", value=f"{accuracy*100:.2f}%", help="Mede a porcentagem de previsões corretas do modelo em todo o conjunto de dados.")
    ...
    if st.session_state.prediction_result is not None:
        ...
        with center_col:
            ...
            with st.expander("🔎 Clique para ver a análise detalhada dos seus hábitos"):
                result_map_original = {
                    'Normal_Weight': 'Peso Normal', 'Overweight_Level_I': 'Sobrepeso Nível I',
                    'Overweight_Level_II': 'Sobrepeso Nível II', 'Obesity_Type_I': 'Obesidade Tipo I',
                    'Obesity_Type_II': 'Obesidade Tipo II', 'Obesity_Type_III': 'Obesidade Tipo III',
                    'Insufficient_Weight': 'Peso Insuficiente'
                }
                
                result_map_from_portuguese = {v: k for k, v in result_map_original.items()}
                ...
                df_proba.columns = ['Classe', 'Probabilidade']
...
# Se o modo for "Painel Analítico", apenas exibe o dashboard.
